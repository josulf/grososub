%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                           %
%  Karrera Bukaerako Proiekturako Plantilla %
%                                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%
% CONFIG: dokumentu estiloa definitu
%

\documentclass[a4paper,12pt,basque]{itsas_pfc}

%
% CONFIG: konfigurazio opzionala
%

\input{Config/pfc_options.tex}

%
% Dokumentua berau hasten da
%

\begin{document}

%
% CONFIG: zenbait aldagairi balioa eman
%

\input{Config/rename.tex}

%         
% EDUKINA: Portadaren edukina.
%         

\input{Config/cover.tex}

%
% CONFIG: orriak 1 zenbakiaz kontatzen hasi.
%

\input{Config/begin.tex}

%
% EDUKINA: eskertzak/zitak/aitzinsolasa gehitu nahi izan ezkero.
%

%\input{file-acknowledgements.tex} % eskertzak
%\input{file-preface.tex}          % aitzinsolasa

%
% CONFIG: Gaien Indizea (ToC).
% 

\input{Config/toc.tex}

%
% CONFIG: goiburu/oinaren estiloa definitzeko.
%

\input{Config/headers.tex}

%
% CONTENT: Laburpena, balego.
%

\input{file-abstract.tex}

%
% CONFIG: Kapituluen estiloa.
%

\input{Config/chapters.tex}

%
% EDUKINA: Honen ostean, gehitu nahi bezainbeste kapitulu/sekzio.
%

\input{file-chapter1.tex} % 1. Kapitulua: Sarrera
\input{file-chapter2.tex} % 2. Kapitulua: Aurrekarien analisia
\input{file-chapter3.tex} % 3. Kapitulua: Proiektuaren helburu dokumentua
\input{file-chapter4.tex} % 4. Kapitulua: Softwarearen garapenerako prozesu bateratua
\input{file-chapter5.tex} % 5. Kapitulua: Hobekuntzak
\input{file-chapter6.tex} % 6. Kapitulua: Ondorioak

%\input{file-example.tex}
%
% CONFIG: Apendizeen estiloa.
%

\input{Config/appendices.tex}

%
% EDUKINA: Apendizeak, baleude.
%

\input{file-appendix1.tex} % I apendizea: Azpititulu formatuak
\input{file-appendix2.tex} % II apendizea: Eskuliburua

%
% CONFIG: Bibliografia estiloa.
%

\input{Config/bibliography.tex}

%
% EDUKINA: Bibliografia. Idatzi sartu nahi dituzun .bib fitxeroen izena, erakusten den moduan.
%

\bibliography{file-bibliography}

%
% EDUKINA: Publikazio zerrenda, balego.
%

%\input{file-publications.tex}

%
% Agur, Ben-Hur.
%

\end{document}
